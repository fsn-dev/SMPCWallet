<template>
  <div class="boxConntent1">
      ...
      <div class="receiveAddress_box">
        <el-form label-position="top" label-width="80px" @submit.native.prevent>
          ...
          <el-form-item label="Advanced:">
            <el-switch v-model="selfSet" :active-value="true" :inactive-value="false"></el-switch>
          </el-form-item>
          <div v-if="selfSet">
            ...
            <el-form-item label="Hex Data：" v-if="selectData.coinType === $$.config.initCoin">
              <el-input v-model="rawTx.data" placeholder="0x"></el-input>
            </el-form-item>
            <el-form-item label="Signed Transaction：" v-if="serializedTxView">
              <el-col :span="13">
                <el-input type="textarea" v-model="signTx" placeholder="0x" :rows="12" style="margin-top:8px"></el-input>
              </el-col>
      ...
      <div class="tableHistory_box">
        ...
        <div class="tableHistory_table table-responsive"
          ...
          <div class="flex-ec mt-20" v-if="pageInfo.total > 1">
            <el-pagination @prev-click="prevClick" @next-click="nextClick" :page-size="pageInfo.pageNum" :pager-count="5" layout="prev, pager, next" :total="pageInfo.total">
            </el-pagination>
          </div>
        </div>
      </div>
    ...
    <el-dialog title="Send Ether & Tokens" :visible.sync="privateSureVisible" width="300" :before-close="modalClick">
      <pwdSure @sendSignData="getSignData" :sendDataPage="rawTx" @elDialogView="getElDialogView" v-if="privateSureVisible"></pwdSure>
    </el-dialog>

    <el-dialog :title="$t('TITLE').TO_SEND" :visible.sync="sendInfoVisible" width="60%" :before-close="modalClick">
      <div class="sendInfo_box">
        <ul>
          ...
          <li><h3>Data:</h3><span>{{rawTx.data || "(none)"}}</span></li>
        </ul>
      </div>
