...
const AddPersonSigns = (params) => {
  return new Promise((resolve, reject) => {
    ...
    db.historyPersonSigns.insert(query, (err, res) => {
      if (err) {
        // console.log(err)
        data.error = err
        reject(data)
...
const EditPersonSigns = (params) => {
  return new Promise((resolve, reject) => {
    ...
    db.historyPersonSigns.update(query, {$set: updateParams}, {}, (err, res) => {
      if (err) {
        // console.log(err)
        data.error = err
        reject(data)
...
const FindPersonSigns = (params) => {
  return new Promise((resolve, reject) => {
    ...
    db.historyPersonSigns.count(query, (err, count) => {
      if (err) {
        data.error = err
        reject(data)
      } else {
        db.historyPersonSigns.find(query).sort({ timestamp: -1 }).skip(Number(_params.skip)).limit(Number(_params.pageSize)).exec((err, res) => {
          if (err) {
            data.error = err
            reject(data)
          } else {
