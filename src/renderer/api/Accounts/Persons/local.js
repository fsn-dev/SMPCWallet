import {historyPersonAccpunts} from '@/db/index.js'
const AddPersonAccounts = (that, url, params) => {
  return new Promise((resolve, reject) => {
    ...
    historyPersonAccpunts.insert(query, (err, res) => {
      if (err) {
        // console.log(err)
        data.error = err
        reject(data)
...
const EditPersonAccounts = (that, url, params) => {
  return new Promise((resolve, reject) => {
    ...
    historyPersonAccpunts.update(query, {$set: updateParams}, {}, (err, res) => {
      if (err) {
        // console.log(err)
        data.error = err
        reject(data)
...
const FindPersonAccounts = (that, url, params) => {
  return new Promise((resolve, reject) => {
    ...
    // console.log(query)
    historyPersonAccpunts.count(query, (err, count) => {
      // console.log(count)
      if (err) {
        data.error = err
        reject(data)
      } else {
        historyPersonAccpunts.find(query).sort({ timestamp: -1 }).skip(Number(_params.skip)).limit(Number(_params.pageSize)).exec((err, res) => {
          if (err) {
            data.error = err
            reject(data)
          } else {
